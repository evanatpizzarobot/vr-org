// Pure aggregation for the /api/ai-stats report. Lives outside the route
// because a Next.js route file may only export route handlers and config:
// exporting a helper from route.ts fails the build with "does not satisfy the
// constraint { [x: string]: never }". Mirrors src/lib/stats/aggregate.ts,
// which /api/stats already uses. All file I/O stays in the route.
...
export async function generateAiStatsFromLines(
...

  {
    for await (const line of lines) {
      ...
      // revalidated). Redirect hops (3xx) and errors (4xx/5xx) are not pageviews
      // of the target, so a scanner hammering a non-existent path, or an
      // http->https / trailing-slash redirect, never inflates a "top page".
